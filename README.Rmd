...
With default settings, performance of `{tidyods}` functions is marginally slower
for small files than `{readODS}`, but this is unlikely to be noticeable to users.
When working with larger files performnace is comparable.
...
Government on the [number of civil servants by postcode](https://www.gov.uk/government/statistics/number-of-civil-servants-by-postcode-department-responsibility-level-and-leaving-cause-2021) (5,544 rows by 11 columns). Analysis 
of read times indicates similar performance to that achieved by `{readODS}`.
...
Due to the reliance on `{xml2}` very large files will perform poorly or run into
memory limitations. This is being investigated as part of the package development.
